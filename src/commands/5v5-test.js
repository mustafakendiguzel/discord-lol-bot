...
async function run({ interaction }) {
  ...
  const voiceChannelId = "1172300076501180430";
  const voiceChannel = await message.guild.channels.fetch(voiceChannelId);

  if (!voiceChannel.members) {
    return message.reply('Could not find the voice channel or it is not a voice channel.');
  }
  ...
  members.forEach(member => {
    userSelections[member.user.id] = "true";
  })
  ...
  collector.on("collect", async (i) => {
    ...
    let updatedPlayerList =
      "@everyone Toplama süresi devam ediyor. Şu anki katılan oyuncular:\n";
